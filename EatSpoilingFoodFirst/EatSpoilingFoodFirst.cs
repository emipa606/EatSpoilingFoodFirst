    ...
    static class FoodUtility_FoodOptimality_Patch
    {
            ...
            ref float __result)
        {
            ...
            if (comp != null)  
            {
                ...
                // if (!takingToInventory && comp.Stage == RotStage.Fresh && comp.TicksUntilRotAtCurrentTemp > 180000)__result -= (5f);  // adds negative offset
                ...
                // Log.Message("FoodOptimality debug (eater:" + eater.Name + "/food:" + foodDef.defName + ") : original =" + original + " result =" + __result + " bonus offset =" + (Math.Abs(__result) - Math.Abs(original)) + " TicksUntilRotAtCurrentTemp = " + comp.TicksUntilRotAtCurrentTemp + " Dist = " + dist);
            }
        }
    }
}
